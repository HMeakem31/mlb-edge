...
:: Step 1: extract the embedded PS1 script
:: for loop overwrites variable -- last match = actual boundary line
set "PS1=%TEMP%\_mlb_extract.ps1"
for /f "tokens=1 delims=:" %%a in ('findstr /n /c:"===PS1_MARKER_a7f3===" "%~f0"') do set /a PSLINE=%%a+1
more +%PSLINE% "%~f0" > "%TEMP%\_ps1_b64.txt"
certutil -decode "%TEMP%\_ps1_b64.txt" "%PS1%" >nul 2>&1
del "%TEMP%\_ps1_b64.txt" 2>nul

:: Step 2: run PS1 -- pass payload marker as arg
if exist "%PS1%" (
...
for /f "tokens=1 delims=:" %%a in ('findstr /n /c:"===PAYLOAD_MARKER_b9d2===" "%~f0"') do set /a SKIP=%%a+1
